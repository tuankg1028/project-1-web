...
const trainingAndTestingPath = "./traningAndTesting (centroid & range)";
...
async function main() {
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    for (const categoryName in groupCategory) {
      ...
      // get value of ranges
      const ranges = getValueOfRanges(setTrainingOfCategory);
      ...
      setTraining.push({
        ...
        category: JSON.stringify(ranges)
      }); // ***
      ...
      for (let j = 0; j < setTestingOfCategory.length; j++) {
        ...
        // counting risk by centroid
        riskPrediction = closestClustertWithTesting.cluster;
        if (closestClustertWithTesting.cluster == testing.feedBack) {
          numberOfCorrentRediction++;
          isCorrect = true; // ***
        } else {
          // counting risk by ranges
          const rangeOfDistance = getRangeOfClusterByDistance(
            testing.distance,
            ranges
          );
          riskPrediction = Object.keys(rangeOfDistance)[0];
  ...
  fs.writeFileSync("./prediction (centroid & range).txt", content);
  console.log("DONE");
}
