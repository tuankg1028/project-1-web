...
const getPredictModel2 = async (tranningAppIds, userAnswer, question) => {
  const tranningApps = await Promise.all(
...
const getPredictModel3 = async (tranningAppIds, userAnswer, question) => {
  const tranningApps = await Promise.all(
    tranningAppIds.map(appId =>
      Models.App.findById(appId).cache(60 * 60 * 24 * 30)
    )
  ...
  const tranningSet = tranningApps.map(tranningApp => {
    ...
    return [
      ...
      ...thirdPartyDataInit.map(item =>
        _.includes(item, flattenThirdPartyData) ? "1" : "0"
      ),
      label.toString()
    ];
  });
...
const getPredictModel5 = async (tranningAppIds, userAnswer, question) => {
  const tranningApps = await Promise.all(
    tranningAppIds.map(appId =>
      Models.App.findById(appId).cache(60 * 60 * 24 * 30)
    )
